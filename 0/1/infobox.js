...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "𝙈𝙧𝙏𝙤𝙢𝙓𝙭𝙓",
  description: "عرض معلومات صندوقك",
...
module.exports.run = async function({ api, event, args, Users }) {
  ...
  ctx.fillText(`⊶ المشرف: ${qtv}`, 439, 243);
  ctx.fillText(`⊶ الذكور: ${nam}`, 439, 287);
  ctx.fillText(`⊶ الإناث: ${nu}`, 439, 331);
  ctx.fillText(`⊶ مجموع الرسائل: ${sl}`, 439, 379);
  ctx.font = `${fontsOthers}px Lobster`;
  ctx.fillStyle = "#00FF00";
  ctx.textAlign = "start";
  ctx.fillText(`ID صندوق: ${id}`, 18, 470);
  ctx.fillText(`• بالاشتراك مع ${parseInt(threadMem) - 3} أعضاء آخرين...`, 607, 453);
  ctx.beginPath();
