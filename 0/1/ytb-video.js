module.exports.config = {
	...
	hasPermssion: 0,
	credits: "𝙈𝙧𝙏𝙤𝙢𝙓𝙭𝙓",
	description: "تشغيل الفيديو عبر رابط YouTube أو كلمة البحث",
	...
	try {
		ytdl.getInfo(handleReply.link[event.body - 1]).then(res => {
			...
			ytdl(handleReply.link[event.body - 1])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`).size > 26214400) return api.sendMessage('لا يمكن إرسال الملف لأن حجمه أكبر من 25 ميجابايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`), event.messageID);
...
module.exports.run = async function ({ api, event, args }) {
	...
	if (args.length == 0 || !args) return api.sendMessage('لا يمكن ترك خانة البحث فارغة!', event.threadID, event.messageID);
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${id}.mp4`).size > 26214400) return api.sendMessage('لا يمكن إرسال الملف لأن حجمه أكبر من 25 ميجابايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.mp4`), event.messageID);
		...
		try {
			...
			var body = `✅ انتهى! ${link.length} نتائج تطابق كلمة البحث الخاصة بك:\n👇👇👇👇👇\n${msg}\nيرجى الرد (التعليق) اختيار واحد من البحوث أعلاه`
			...
			var body = `✅ انتهى! ${link.length} نتائج تطابق مصطلح البحث الخاص بك:\n👇👇👇👇👇\n${msg}\nيرجى الرد (التعليق) بالاختيار من بين البحوث أعلاه`
			return api.sendMessage({ attachment: imgthumnail, body: body }, event.threadID, (error, info) => global.client.handleReply.push({
				name: this.config.name,
				messageID: info.messageID,
				author: event.senderID,
