...
module.exports.run = async function({ api, event, args, Users, Threads, Currencies }) {
  const axios = require("axios");
    ...
    var tl = ['21%', '67%', '19%', '37%', '17%', '96%', '52%', '62%', '76%', '83%', '100%', '99%', "0%", "48%"];
        ...
        var gender = sex == 2 ? "الذكر🧑" : sex == 1 ? "الأنثى👩‍  " : "TRAN Duc Bo";
var one = senderID, two = id;
    return makeImage({ one, two }).then(path => api.sendMessage({ body: `تهانينا ${namee} قد تم شحنه مع ${name}\n\nنسبة الرومانسية: ${tle}`, mentions: arraytag, attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
                                          }